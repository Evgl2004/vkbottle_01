...
1. Регистрация и legacy-анкета в VK по-прежнему частично работают через сообщения и payload (`private_message`), а не через полный callback+edit-цикл.
2. Для шага с вводом текста (имя, фамилия, email, ответы в тикетах) в VK неизбежно используются новые сообщения пользователя, поэтому абсолютная «экранность» как в Telegram ограничена платформой.
3. Сценарий виртуальной карты отправляет отдельные сообщения с изображениями QR (что ближе к VK-практике вложений), а не редактирует одно сообщение на весь поток.
